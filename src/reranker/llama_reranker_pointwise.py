...
class LlamaRerankerPointwise(Transformer):
    ...
    def transform(self, topics_or_res: pd.DataFrame) -> pd.DataFrame:
        prompts = topics_or_res.apply(lambda x : PointwisePrompt(query=x.query, documents=[x.text]).generate()[0], axis=1) # Point wise prompt creation
...
class LlamaRerankerPointwiseKShots(LlamaRerankerPointwise):
    ...
    def transform(self, topics_or_res: pd.DataFrame) -> pd.DataFrame:
        prompts = topics_or_res.apply(lambda x : PointwisePrompt(query=x.query, documents=[x.text], is_k_shots=True, 
...
if __name__== "__main__" :
    ...
    pointwise_prompt = PointwisePrompt(query=query, documents=documents)
    prompts = pointwise_prompt.generate()
    #print(prompts)
    ...
    llama_reranker = LlamaRerankerPointwise(model=LlamaForCausalLM.from_pretrained('/llm/llama2/llama2_7b', device_map="auto"), 
                                    tokenizer=tokenizer)
