...
Early information retrieval systems, such as those based on Boolean logic models, were effective in specific application scenarios but limited in understanding the deeper semantics of text. As a milestone, the BM25 probabilistic model, proposed by Stephen Robertson and others at the TREC-3 conference, considered term frequency (TF) and inverse document frequency (IDF) along with document length, providing a probabilistic framework for relevance assessment. BM25 improved retrieval quality over its predecessors, yet it still lacked in dealing with the depth of word meanings, especially synonyms and contextual semantics.

To address these issues, Vector Space Models (VSM) and Latent Semantic Analysis (LSA) were introduced. VSM represents text content as vectors in a multidimensional space and uses cosine similarity to assess similarity between documents, offering a new method for document ranking. LSA, through singular value decomposition (SVD), further reveals the latent semantic structure between documents. Although this improved the models' understanding of semantic relationships, limitations remained in handling semantic diversity.

In recent years, the Transformer architecture and its derivatives, such as BERT (Bidirectional Encoder Representations from Transformers) and GPT (Generative Pre-trained Transformer), have effectively uncovered the subtle semantic connections between query intentions and document content through self-attention mechanisms. These models have shown potential improvements in the performance of information retrieval systems, particularly in relevance assessment and document ranking. For instance, research by Nogueira et al. utilized BERT to re-rank preliminary search results, demonstrating the potential of deep learning models to enhance retrieval system performance. They introduced a two-stage ranking framework, decomposing document ranking into two phases. The first phase uses the classical BM25 probabilistic model to quickly filter a candidate document set with higher relevance from a large dataset. The second phase employs deep learning-based machine learning models for in-depth semantic analysis and re-ranking of the candidate list from the first phase, thus providing a more accurate relevance assessment and ranking of candidate documents.
